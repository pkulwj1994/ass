...
from losses.vae import elbo, elbo_ssm, elbo_ass, iwae, elbo_kernel
from models.vae import Encoder, Decoder, ImplicitEncoder, MLPEncoder, MLPDecoder, \
    MLPImplicitEncoder, MLPScore, Score, MLPScore_ass, Score_ass
import itertools
...
class VAERunner():
    ...
    def train(self):
        ...
        elif self.config.training.algo == 'ass':
            score = MLPScore_ass(self.config).to(self.config.device) if self.config.data.dataset == 'MNIST' else \
                Score_ass(self.config).to(self.config.device)
            imp_encoder = MLPImplicitEncoder(self.config).to(self.config.device) if self.config.data.dataset == 'MNIST' \
                else ImplicitEncoder(self.config).to(self.config.device)
            opt_decoder = optim.RMSprop(decoder.parameters(), lr=self.config.optim.decoder_lr)
            opt_encoder = optim.RMSprop(imp_encoder.parameters(), lr=self.config.optim.encoder_lr)
            opt_score = optim.RMSprop(score.parameters(), lr=self.config.optim.score_lr)
            if self.args.resume_training:
                states = torch.load(os.path.join(self.args.log, 'checkpoint.pth'))
                imp_encoder.load_state_dict(states[0])
                decoder.load_state_dict(states[1])
                score.load_state_dict(states[2])
                opt_decoder.load_state_dict(states[3])
                opt_encoder.load_state_dict(states[4])
                opt_score.load_state_dict(states[5])
        ...
        for _ in range(self.config.training.n_epochs):
            for _, (X, y) in enumerate(dataloader):
                ...
                if self.config.training.algo == 'vae':
                    ...
                    imp_encoder.train()
                    loss, encoder_loss, score_loss = elbo_ass(imp_encoder, opt_encoder, decoder, score, opt_score, X, 
                                                self.args, recon_type, training=True)
                ...
                if step % 10 == 0:
                    ...
                    if self.config.training.algo == 'vae':
                        ...
                        imp_encoder.eval()
                        test_loss, *_ = elbo_ass(imp_encoder, None, decoder, score, None, X, 
                                                self.args, recon_type, training=False)
                        logging.info("loss: {}, encoder_loss: {}, score_loss: {}, test_loss: {}".format(loss.item(), encoder_loss.item(),
                                                                                    score_loss.item(),test_loss.item()))
                        z = imp_encoder(test_X)
                        tb_logger.add_histogram('z_X', z, global_step=step)
                    ...
                    if self.config.training.algo == 'ssm':
                        ...
                        tb_logger.add_scalar('encoder_loss', encoder_loss, global_step=step)
                        tb_logger.add_scalar('score_loss', score_loss, global_step=step)
                ...
                if (step + 1) % 10000 == 0:
                    if self.config.training.algo == 'vae':
                        ...
                        states = [
                            ...
                            decoder.state_dict(),
                            score.state_dict(),
                            opt_decoder.state_dict(),
                            opt_encoder.state_dict(),
                            opt_score.state_dict()
                        ]
                    torch.save(states,
    ...
    def test(self):
        ...
        # MC correction steps
        if self.config.training.algo == 'ass':
            print("MC correction steps")
